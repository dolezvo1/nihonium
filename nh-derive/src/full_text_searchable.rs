...
fn derive_enum(
    input_ast: &syn::DeriveInput,
    opts: &DeriveFullTextSearchableOpts,
    ...
    let arms = data_enum
        .variants
        .iter()
        .flat_map(|v| DeriveFullTextSearchableVariantOpts::from_variant(v).map(|e| (v, e)))
        .collect::<Vec<_>>();
    let arms2 = arms
        .iter()
        .map(|e| {
    ...
    let arms_immutable = arms2
        ...
        .collect::<Vec<_>>();
    let arms_search = arms_immutable
        .iter()
        .map(|e| quote! { #e.full_text_search(acc) })
        .collect::<Vec<_>>();
...
fn derive_struct(
    ...
    let searched_values = data_struct
        .fields
        .iter()
        .filter_map(|e| {
            DeriveFullTextSearchableFieldOpts::from_field(e)
                .map(|o| {
                    let field = &e.ident;
                    if o.skip {
                        quote! {}
