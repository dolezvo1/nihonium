...
pub fn derive_view(input: TokenStream) -> TokenStream {
    ...
    let arms = data_enum
        .variants
        .iter()
        .flat_map(|v| DeriveViewVariantOpts::from_variant(v).map(|e| (v, e)))
        .collect::<Vec<_>>();
    let arms2 = arms
        .iter()
        .map(|e| {
    ...
    let arms_uuid = arms_immutable
        ...
        .map(|e| quote! { #e.uuid() })
        .collect::<Vec<_>>();
    let arms_model_uuid = arms_immutable
        .iter()
        .map(|e| quote! { #e.model_uuid() })
        .collect::<Vec<_>>();

    let arms_model = arms_immutable
        .iter()
        .map(|e| quote! { #e.model() })
        .collect::<Vec<_>>();
    let arms_min_shape = arms_immutable
        .iter()
        .map(|e| quote! { #e.min_shape() })
        .collect::<Vec<_>>();
    let arms_bounding_box = arms_immutable
        .iter()
        .map(|e| quote! { #e.bounding_box() })
        .collect::<Vec<_>>();
    let arms_position = arms_immutable
        .iter()
        .map(|e| quote! { #e.position() })
        .collect::<Vec<_>>();

    let arms_show_properties = arms_mutable
        .iter()
        .map(|e| quote! { #e.show_properties(gdc, q, ui, commands) })
        .collect::<Vec<_>>();
    let arms_draw_in = arms_mutable
        .iter()
        .map(|e| quote! { #e.draw_in(q, context, settings, canvas, tool) })
        .collect::<Vec<_>>();
    let arms_collect_alignment = arms_mutable
        .iter()
    ...
    let arms_delete_when = arms_immutable
        .iter()
        .map(|e| quote! { #e.delete_when(deleting) })
        .collect::<Vec<_>>();
    let arms_deep_copy_walk = arms_immutable
        .iter()
        .map(|e| quote! { #e.deep_copy_walk(requested, uuid_present, tlc, c, m) })
        .collect::<Vec<_>>();
    let arms_deep_copy_clone = arms_immutable
        .iter()
        .map(|e| quote! { #e.deep_copy_clone(uuid_present, tlc, c, m) })
        .collect::<Vec<_>>();
    let arms_deep_copy_relink = arms_mutable
        .iter()
        .map(|e| quote! { #e.deep_copy_relink(c, m) })
        .collect::<Vec<_>>();
