...
use crate::common::search::{ModelSearchResults, ResourceSearchResults};
...
impl NHTab {
    pub fn name<'a>(&self, gdc: &'a GlobalDrawingContext) -> Cow<'a, str> {
        match self {
            ...
            NHTab::ModelHierarchy => gdc.translate_0("nh-tab-modelhierarchy"),
...
struct NHContext {
    ...
    search_query: String,
    search_error: String,
    model_search_results: ModelSearchResults,
    resource_search_results: ResourceSearchResults,
...
impl NHContext {
    ...
    fn show_search(&mut self, ui: &mut egui::Ui) {
            ...
            .changed()
        {
            if self.search_query.is_empty() {
                self.model_search_results = Default::default();
                self.resource_search_results = Default::default();
                return;
            }
        ...
        egui_ltreeview::TreeView::new(egui::Id::new("search results")).show(ui, |builder| {
            self.model_search_results.show(
                builder,
                &self.drawing_context,
            ...
            self.resource_search_results.show(
                builder,
                &self.drawing_context,
                &mut self.unprocessed_commands,
            );
    ...
    fn last_focused_diagram(&self) -> Option<(ViewUuid, ERef<dyn DiagramController>)> {
        self.last_focused_diagram
            .as_ref()
            .and_then(|e| self.diagram_controllers.get(e).map(|c| (*e, c.clone())))
    }
}
...
impl NHApp {
    ...
    fn new(
        ...
        let mut context = NHContext {
            ...
            model_search_results: Default::default(),
            resource_search_results: Default::default(),
...
impl NHApp {
    ...
    fn add_diagram(
        ...
        if let Err(e) = self.context.project_hierarchy.insert(
            ...
            let HierarchyNode::Folder(.., children) = &mut self.context.project_hierarchy else {
                unreachable!()
            };
            children.push(e);
        }
    ...
    fn delete_elements(&mut self, targets: &HashSet<EntityUuid>) {
        ...
        all_to_delete.remove(&FolderUuid::nil().into());
...
impl eframe::App for NHApp {
    ...
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        macro_rules! translate {
            ($msg_name:expr) => {
                self.context.drawing_context.translate_0($msg_name)
            };
        }
                    ...
                    for tab in &[
                        ...
                        if ui
                            .selectable_label(
                                ...
                                tab.name(&self.context.drawing_context),
                            )
                            .clicked()
                        {
        ...
        if let Some(confirm_reason) = &self.context.confirm_modal_reason {
            macro_rules! unselected_deletes {
                ($ui:expr, $u:expr, $m1:expr, $m2:expr, $v:expr, $cmd:expr, $dont_ask_again_default:expr, $dont_ask_again_target:expr) => {
                    ...
                    $ui.label(format!("This operation would also delete other (unselected) elements (and their children) (see below). How would you like to continue?"));
                    $ui.add_space(SPACING);
                    $ui.collapsing(
                        format!("Unselected model elements ({})", $m1.len()),
                        |ui| {
                            for e in $m1 {
                    ...
                    $ui.collapsing(
                        format!("Other model elements ({})", $m2.len()),
                        |ui| {
                            for e in $m2 {
                                ui.label(&*self.context.drawing_context.model_labels.get(e));
                            }
                            ...
                            DiagramCommand::DeleteSelectedElements(k, u) => {
                                match (
                                    ...
                                    (otherwise, u) => {
                                        ...
                                        let (m1, m2, v) = ac.read().calculate_unintended_deletes(
                                            uuid,
