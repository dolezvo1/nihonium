...
impl<P: UmlClassProfile> DiagramSettings for UmlClassSettings<P> {
                        ...
                        match tool {
                            ...
                            UmlClassToolStage::Class {
                                ...
                                modified |= columns[1]
                                    ...
                                    .changed();
                                modified |=
                                    columns[1].checkbox(is_abstract, "isAbstract").changed();
    ...
    fn clone_reduced(&self) -> Box<dyn DiagramSettings> {
        Box::new(Self {
            ...
            instance_buttons: Vec::new(),
            class_buttons: Vec::new(),
            comment_indication: self.comment_indication,
            visibility_style: self.visibility_style,
        })
    }
    ...
    fn add_section(&mut self, element: UmlClassElement) {
        ...
        match element {
            ...
            UmlClassElement::Instance(inner) => match (&self.current_stage, &mut self.result) {
                ...
                (UmlClassToolStage::LinkEnd, PartialUmlClassElement::Link { dest, .. })
                    if let UmlClassToolStage::LinkStart { link_type } = &self.initial_stage =>
                {
                    if !matches!(link_type, LinkType::Generalization { .. }) {
                        *dest = Some(inner.into());
                    }
            ...
            UmlClassElement::Class(inner) => match (&self.current_stage, &mut self.result) {
                ...
                (UmlClassToolStage::LinkEnd, PartialUmlClassElement::Link { source, dest })
                    if let UmlClassToolStage::LinkStart { link_type } = &self.initial_stage =>
                {
                    if !matches!(link_type, LinkType::Generalization { .. })
    ...
    fn try_flush(
        ...
        match &mut self.result {
            ...
            PartialUmlClassElement::Package { a, b: Some(b) }
                if let UmlClassToolStage::PackageStart {
                    name,
                    stereotype,
                    kind,
                } = &self.initial_stage =>
...
impl<P: UmlClassProfile> ElementControllerGen2<UmlClassDomain<P>> for UmlClassInstanceView<P> {
    ...
    fn apply_command(
        &mut self,
        _diagram_model: &ERef<UmlClassDiagram>,
        command: &InsensitiveCommand<
            UmlClassOrdinalMovement,
            UmlClassElementOrVertex<P>,
            UmlClassPropChange,
                ...
                if uuids.contains(&*self.uuid) {
                    ...
                    match property {
                        ...
                        UmlClassPropChange::IsStaticChange(is_static) => {
                            undo_accumulator.push(InsensitiveCommand::PropertyChange(
                                std::iter::once(*self.uuid).collect(),
                                UmlClassPropChange::IsStaticChange(model.is_static),
                            ));
                        ...
                        UmlClassPropChange::IsDerivedChange(is_derived) => {
                            undo_accumulator.push(InsensitiveCommand::PropertyChange(
                                std::iter::once(*self.uuid).collect(),
                                UmlClassPropChange::IsDerivedChange(model.is_derived),
                            ));
                            model.is_derived = *is_derived;
                        }
                        UmlClassPropChange::IsReadOnlyChange(is_read_only) => {
                            undo_accumulator.push(InsensitiveCommand::PropertyChange(
                                std::iter::once(*self.uuid).collect(),
                                UmlClassPropChange::IsReadOnlyChange(model.is_read_only),
                            ));
                        ...
                        UmlClassPropChange::IsOrderedChange(is_ordered) => {
                            undo_accumulator.push(InsensitiveCommand::PropertyChange(
                                std::iter::once(*self.uuid).collect(),
                                UmlClassPropChange::IsOrderedChange(model.is_ordered),
                            ));
                            model.is_ordered = *is_ordered;
                        }
                        UmlClassPropChange::IsUniqueChange(is_unique) => {
                            undo_accumulator.push(InsensitiveCommand::PropertyChange(
                                std::iter::once(*self.uuid).collect(),
                                UmlClassPropChange::IsUniqueChange(model.is_unique),
                            ));
                            model.is_unique = *is_unique;
                        }
                        UmlClassPropChange::IsIdChange(is_id) => {
                            undo_accumulator.push(InsensitiveCommand::PropertyChange(
                                std::iter::once(*self.uuid).collect(),
                                UmlClassPropChange::IsIdChange(model.is_id),
                            ));
                            model.is_id = *is_id;
                        }
                        _ => {}
                ...
                if uuids.contains(&*self.uuid) {
                    ...
                    match property {
                        ...
                        UmlClassPropChange::IsAbstractChange(is_abstract) => {
                            undo_accumulator.push(InsensitiveCommand::PropertyChange(
                                std::iter::once(*self.uuid).collect(),
                                UmlClassPropChange::IsAbstractChange(model.is_abstract),
                            ));
                        ...
                        UmlClassPropChange::IsQueryChange(is_query) => {
                            undo_accumulator.push(InsensitiveCommand::PropertyChange(
                                std::iter::once(*self.uuid).collect(),
                                UmlClassPropChange::IsQueryChange(model.is_query),
                            ));
                            model.is_query = *is_query;
                        }
                        UmlClassPropChange::IsOrderedChange(is_ordered) => {
                            undo_accumulator.push(InsensitiveCommand::PropertyChange(
                                std::iter::once(*self.uuid).collect(),
...
impl<P: UmlClassProfile> ElementControllerGen2<UmlClassDomain<P>> for UmlClassView<P> {
    ...
    fn apply_command(
        &mut self,
        diagram_model: &ERef<UmlClassDiagram>,
        command: &InsensitiveCommand<
            UmlClassOrdinalMovement,
            UmlClassElementOrVertex<P>,
            UmlClassPropChange,
                        ...
                        match e {
                            UmlClassElementView::ClassProperty(inner) => {
                                let view_pos = |arr: &Vec<ERef<UmlClassPropertyView<_>>>| {
                                    ...
                                    for e in arr {
                                        let Some((_b, pos)) = self
                                            .model
                                            .read()
                                            .get_element_pos(&e.read().model_uuid())
                                        else {
                                            unreachable!()
                                        };
                                        if pos < model_pos {
        ...
        if let InsensitiveCommand::PropertyChange(_, property) = command {
            ...
            match property {
                ...
                UmlClassPropChange::SetCoveringChange(is_covering) => {
                    undo_accumulator.push(InsensitiveCommand::PropertyChange(
                        std::iter::once(*view_uuid).collect(),
                        UmlClassPropChange::SetCoveringChange(model.set_is_covering),
                    ));
                    model.set_is_covering = *is_covering;
                }
                UmlClassPropChange::SetDisjointChange(is_disjoint) => {
                    undo_accumulator.push(InsensitiveCommand::PropertyChange(
                        std::iter::once(*view_uuid).collect(),
                        UmlClassPropChange::SetDisjointChange(model.set_is_disjoint),
                    ));
                    model.set_is_disjoint = *is_disjoint;
                }
                UmlClassPropChange::CommentChange(comment) => {
                    undo_accumulator.push(InsensitiveCommand::PropertyChange(
                        std::iter::once(*view_uuid).collect(),
